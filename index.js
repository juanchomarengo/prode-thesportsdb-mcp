...
// Remote (Streamable HTTP) MCP server that wraps TheSportsDB V2 API and exposes
// a small set of football tools tailored to predicting World Cup 2026 scorelines.
//
...
function buildServer() {
  ...
  // 1) Resolve a team name to its TheSportsDB id.
  server.tool(
    "find_team",
  ...
  // 2) Recent form (previous finished matches) for a team.
  server.tool(
    "team_recent_form",
    "Recent results (form) for a team by id: last matches with scores, plus a W/D/L + goals summary. Great for estimating attacking/defensive strength.",
    {
      ...
      limit: z.number().int().min(1).max(10).optional().describe("How many recent matches (default 6)"),
    },
    async ({ teamId, limit }) => {
      try {
        const data = await tsdb(`/schedule/previous/team/${encodeURIComponent(teamId)}`);
        const events = rows(data).map(compactEvent);
        const n = limit || 6;
        const recent = events.slice(0, n);
        return text({ teamId, summary: formSummary(teamId, recent), matches: recent });
      } catch (e) {
        return fail(String(e.message || e));
      }
    }
  ...
  server.tool(
    ...
    async ({ teamId }) => {
      try {
        const data = await tsdb(`/schedule/next/team/${encodeURIComponent(teamId)}`);
  ...
  // 4) Head-to-head: recent meetings between two teams.
  server.tool(
    "head_to_head",
    ...
    async ({ teamAId, teamBId }) => {
      try {
        const data = await tsdb(`/schedule/previous/team/${encodeURIComponent(teamAId)}`);
  ...
  // 5) Current World Cup 2026 schedule / results (live tournament state).
  server.tool(
    "world_cup_matches",
    ...
    {
      filter: z.enum(["all", "results", "upcoming"]).optional().describe("Default 'all'"),
      team: z.string().optional().describe("Optional team name substring to filter, e.g. 'Argentina'"),
    },
    async ({ filter, team }) => {
      try {
        const data = await tsdb(`/schedule/league/${WC_LEAGUE_ID}/${WC_SEASON}`);
        ...
        return text({ season: WC_SEASON, filter: f, count: events.length, matches: events });
      } catch (e) {
        return fail(String(e.message || e));
      }
    }
  ...
  // 6) Live scores for the World Cup right now.
  server.tool(
    "world_cup_live",
...
// --- HTTP layer (stateless Streamable HTTP transport) ---
const app = express();
app.use(express.json({ limit: "1mb" }));

app.get("/health", (_req, res) => res.json({ ok: true, service: "prode-thesportsdb-mcp" }));
...
// Stateless mode does not support GET (server-initiated SSE) or DELETE.
...
app.listen(PORT, () => {
  console.log(`prode-thesportsdb-mcp listening on :${PORT}  (MCP endpoint: POST /mcp)`);
  console.log(`Auth: ${AUTH ? "ENABLED (Bearer token required)" : "disabled"}`);
});
